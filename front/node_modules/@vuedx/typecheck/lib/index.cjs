...
const parseArgs__default = /*#__PURE__*/_interopDefaultLegacy(parseArgs);

var version = "0.7.6";

function resolve(moduleId, directory) {
    try {
        return resolveFrom__default["default"](directory, moduleId);
    }
    catch {
...
class TypeScriptServerHost {
    constructor() {
        ...
        this.pluginPath = Path__namespace.resolve(Path__namespace.dirname(require.resolve('@vuedx/typescript-plugin-vue/package.json')), '../..');
        this.isClosed = false;
        ...
        const debugArgs = process.env['DEBUG_TS_SERVER'] != null
            ...
            : [];
        // prettier-ignore
        this.server = child_process.fork(this.serverPath, [
            ...debugArgs,
        ...
        this.readline = readline.createInterface({ input: this.stdout });
...
function toNormalizedPath$1(fileName) {
    return TS__namespace.server.toNormalizedPath(fileName);
}
class AbortSignal {
    constructor() {
        this._aborted = false;
...
async function* getDiagnosticsStream(directory, cancellationToken, logging = false, filter = () => true) {
    ...
    let files;
    let projectFileName;
    const jsConfig = Path__namespace.resolve(directory, 'jsconfig.json');
    const tsConfig = Path__namespace.resolve(directory, 'tsconfig.json');
    if (FS__namespace.existsSync(tsConfig) || FS__namespace.existsSync(jsConfig)) {
        projectFileName = FS__namespace.existsSync(tsConfig) ? tsConfig : jsConfig;
        ...
        shared.invariant(body != null, 'Project info is null.');
        shared.invariant(body.fileNames != null, 'Project has no files.');
        ...
        files = (await glob__default["default"](['**/*.vue', '**/*.ts', '**/*.js', '**/*.jsx', '**/*.tsx'], {
            cwd: directory,
            absolute: true,
...
async function getDiagnostics$1(directory) {
    ...
    while ((next = await stream.next()).done === false)
        ;
    shared.invariant(next.done === true, 'Stream should be done.');
    return next.value;
}
function merge(...items) {
    return shared.flatten(items.map((item) => item ?? []));
}
...
const colors = {
    ...
    suggestion: chalk__default["default"].green,
    message: chalk__default["default"].blueBright,
};
...
async function createTextDocument(file) {
    const content = await FS.promises.readFile(Path__namespace.resolve(directory, file), {
        encoding: 'utf-8',
    });
    const fileName = toNormalizedPath(file);
    const document = TextDocument.create(fileName, Path__namespace.posix.extname(file), 0, content);
    cache.set(fileName, document);
    return document;
}
function toNormalizedPath(fileName) {
    return TS__default["default"].server.toNormalizedPath(fileName);
}
function formatLocation(fileName, start) {
    ...
    let output = '';
    output += chalk__default["default"].cyan(relativeFileName);
    output += ':';
    output += chalk__default["default"].yellow(`${line}`);
    output += ':';
    output += chalk__default["default"].yellow(`${column}`);
    return output;
}
...
async function formatDiagnosticsWithColorAndContext(fileName, diagnostic) {
    ...
    output += chalk__default["default"].gray(` ${diagnostic.source ?? ''}${diagnostic.code ?? ''}: `);
    output += diagnostic.text;
    const document = await getTextDocument(fileName);
    output += '\n';
    output += generateCodeFrame(document.getText(), document.offsetAt(toPosition(diagnostic.start)), document.offsetAt(toPosition(diagnostic.end)), (underline) => colors[category](underline), (gutter) => chalk__default["default"].bgWhite(gutter.trimEnd()) + ' ');
    if (diagnostic.relatedInformation != null) {
        for (const { message, span, category } of diagnostic.relatedInformation) {
            ...
            if (span != null) {
                ...
                output += '\n';
                output += generateCodeFrame(document.getText(), document.offsetAt(toPosition(span.start)), document.offsetAt(toPosition(span.end)), (underline) => color(underline), (gutter) => chalk__default["default"].bgWhite(gutter.trimEnd()) + ' ');
                output += '\n';
            }
            output += '    ' + message;
        }
...
function convertToRelativePath(fileName) {
    return Path__namespace.isAbsolute(fileName)
        ? Path__namespace.relative(directory, fileName).replace(/\\/g, '/')
        : fileName;
}
async function _cli() {
    const { pretty, vue, help, watch, format, _: argv, } = parseArgs__default["default"](process.argv.slice(2), {
...
`.trim());
    ...
    directory =
        argv[0] != null
            ? Path__namespace.isAbsolute(argv[0])
                ? argv[0]
                : Path__namespace.resolve(process.cwd(), argv[0])
            : process.cwd();
    ...
    if (!FS__default["default"].statSync(directory).isDirectory()) {
        console.error(`Expecting a directory, but "${process.argv[2] ?? ''}" is not a directory.`);
        process.exit(1);
    }
    shared.collect('cli exec', {
        watch,
        format,
        vue,
        pretty,
    ...
    shared.invariant(result.done, 'Expected done to be true');
    if (format === 'raw') {
        clearScreen();
    }
    await handleResults(result.value, { format, pretty, vue });
...
function setup() {
    shared.Telemetry.setup('https://212f65f46796440ebbe974f24b20ffae@o237831.ingest.sentry.io/5595698', 'typecheck', version, process.env['CI'] != null ? 1 : 0.25, {
        ci: process.env['CI'],
    });
}
